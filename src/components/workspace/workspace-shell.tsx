...
import type { AttachmentPayload, HelpRequestSummary, MessageSummary, WorkspaceData } from "@/lib/types/app";
import { createBrowserSupabaseClient } from "@/lib/supabase/browser";
import { buildCallLink, canManageDepartment, cn } from "@/lib/utils";
...
export function WorkspaceShell({
  ...
  const primaryNav = [
    ...
    { id: "help" as WorkspaceView, label: "Help", icon: HandHelping },
    {
      ...
      label: `Alerts ${data.notifications.filter((notification) => !notification.isRead).length ? `(${data.notifications.filter((notification) => !notification.isRead).length})` : ""}`,
      icon: Bell,
    },
  ];
  const mobileNav = [
    { id: "chat" as WorkspaceView, label: "Chat", icon: MessageCircleMore },
    { id: "assignments" as WorkspaceView, label: "Work", icon: BookCheck },
    { id: "help" as WorkspaceView, label: "Help", icon: HandHelping },
    { id: "notifications" as WorkspaceView, label: "Alerts", icon: Bell },
  ];

  const department = data.departments.find((item) => item.id === departmentId) ?? data.departments[0];
  const thread = data.directThreads.find((item) => item.id === threadId) ?? data.directThreads[0];
  const roomId = roomMode === "department" ? department?.id ?? null : thread?.id ?? null;
  const roomName =
    roomMode === "department"
      ? department?.name ?? "School space"
      : thread?.title ?? thread?.participants.map((person) => person.fullName).join(", ") ?? "Direct thread";
  const canManage = roomMode === "department" && department ? canManageDepartment(department, data.currentUser) : false;
  const messages = data.messages.filter((message) => roomMode === "department" ? message.departmentId === department?.id : message.directThreadId === thread?.id);
  const assignments = data.assignments.filter((assignment) => assignment.departmentId === department?.id);
  const helpRequests = data.helpRequests.filter((helpRequest) => helpRequest.departmentId === department?.id);
  const resources = data.resources.filter((resource) => resource.departmentId === department?.id);
  const announcements = data.announcements.filter((announcement) => announcement.departmentId === department?.id);
  const members = data.directory.filter((entry) => entry.departmentId === department?.id).sort((left, right) => right.user.points - left.user.points);
  ...
  const onIncomingMessage = useEffectEvent(async (messageId: string) => {
    ...
    setData((current) => ({
      ...current,
      messages: [...current.messages.filter((message) => message.id !== row.id), normalizeMessage(row)].sort((a, b) => a.createdAt.localeCompare(b.createdAt)),
    }));
  });
  ...
  useEffect(() => {
    ...
    channel
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages", filter }, (payload) => void onIncomingMessage(String(payload.new.id)))
      .on("broadcast", { event: "typing" }, ({ payload }) => {
        ...
        window.setTimeout(() => setTyping((current) => current.filter((item) => item !== handle)), 1400);
      })
      .subscribe();
    return () => {
      setTyping([]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setData((current) => ({ ...current, messages: [...current.messages, normalizeMessage(row)] }));
      setDraft(""); setMessageFile(null); setReplyToId(null); setNotice(null);
    } catch (error) {
        ...
        <section className="flex min-w-0 flex-1 flex-col gap-4 pb-24 xl:pb-0">
          ...
          <div className="grid min-h-0 flex-1 gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="glass-panel min-h-0 rounded-[2rem] border border-white/55 p-4 dark:border-white/10">
              <nav className="mb-4 flex gap-2 overflow-x-auto">{primaryNav.map((item) => <button key={item.id} type="button" onClick={() => startTransition(() => { if (roomMode === "direct" && item.id !== "chat" && data.departments[0]) { setRoomMode("department"); setDepartmentId(data.departments[0].id); } setView(item.id); })} className={cn("inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold", view === item.id ? "bg-accent text-white dark:bg-white dark:text-black" : "bg-white/70 dark:bg-white/5")}><item.icon className="h-4 w-4" />{item.label}</button>)}</nav>
              {notice ? <div className="mb-3 rounded-[1.1rem] border border-line bg-white/80 px-4 py-3 text-sm text-muted dark:bg-white/6">{notice}</div> : null}
              {view === "chat" ? <WorkspaceChat currentUser={data.currentUser} messages={messages} draft={draft} setDraft={(value) => { setDraft(value); if (roomChannel.current) void roomChannel.current.send({ type: "broadcast", event: "typing", payload: { handle: data.currentUser.handle } }); }} file={messageFile} setFile={setMessageFile} replyToId={replyToId} setReplyToId={setReplyToId} typing={typing} onSend={() => void sendMessage()} onToggleReaction={(messageId, emoji) => void toggleReaction(messageId, emoji)} onReply={setReplyToId} /> : null}
              {view === "assignments" ? <WorkspaceAssignments assignments={assignments} canManage={canManage} currentUserId={data.currentUser.id} assignmentTitle={assignmentTitle} setAssignmentTitle={setAssignmentTitle} assignmentInstructions={assignmentInstructions} setAssignmentInstructions={setAssignmentInstructions} assignmentDueAt={assignmentDueAt} setAssignmentDueAt={setAssignmentDueAt} onCreate={() => void createAssignment()} submissionAssignmentId={submissionAssignmentId} setSubmissionAssignmentId={setSubmissionAssignmentId} submissionBody={submissionBody} setSubmissionBody={setSubmissionBody} onSubmit={() => void submitAssignment()} attachSubmission={setMessageFile} /> : null}
              {view === "help" ? <WorkspaceHelpBoard helpRequests={helpRequests} helpTitle={helpTitle} setHelpTitle={setHelpTitle} helpDescription={helpDescription} setHelpDescription={setHelpDescription} onCreate={() => void createHelpRequest()} onVolunteer={(helpRequestId) => void volunteer(helpRequestId)} onResolve={(helpRequestId) => void resolve(helpRequestId)} currentUserId={data.currentUser.id} canManage={canManage} /> : null}
              {view === "notifications" ? <WorkspaceNotifications notifications={data.notifications} badges={data.badges} onMarkAllRead={() => void markAllRead()} /> : null}
            </div>
            <WorkspaceContextPanel announcements={announcements} resources={resources} members={members} />
          </div>
          <nav className="glass-panel fixed inset-x-4 bottom-4 z-20 grid grid-cols-4 gap-2 rounded-full border border-white/60 p-2 xl:hidden dark:border-white/10">{mobileNav.map((item) => <button key={item.id} type="button" onClick={() => setView(item.id)} className={cn("flex flex-col items-center gap-1 rounded-full px-2 py-2 text-[11px] font-semibold", view === item.id ? "bg-accent text-white dark:bg-white dark:text-black" : "text-muted")}><item.icon className="h-4 w-4" />{item.label}</button>)}</nav>
        </section>
      </div>
    </main>
  );
